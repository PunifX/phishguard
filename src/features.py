...
def fetch_one(url):
    try:
        ...
        return {"total_links": total, "empty_links": empty, "external_links": external,
                "internal_links": internal, "ratio_external_total": ratio,
                "total_forms": total_forms, "suspicious_forms": suspicious_forms}
    except:
        return {"total_links": 0, "empty_links": 0, "external_links": 0,
...
def extraction_hyper_link(urls):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        for i, result in enumerate(executor.map(fetch_one, urls)):
            ...
            if i % 10000 == 0:
                pd.DataFrame(results).to_csv(r"D:\codes\project\AI\improved version of phishguard\output\features\hyper_link_features.csv", index=False)
...
#hyper_link_features = pd.read_csv(r"D:\codes\project\AI\improved version of phishguard\output\features\hyper_link_features.csv")
